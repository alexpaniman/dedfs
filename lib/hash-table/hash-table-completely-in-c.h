...
#define hash_table_pair(K, V) hash_table_pair_##K##_##V
#define hash_table_pair_create(K, V) hash_table_pair_create_##K##_##V

#define hash_table(K, V) hash_table_##K##_#V

#define hash_table_create(K, V) hash_table_create_##K##_##V
#define hash_table_simple_key_equality(K, V) hash_table_simple_key_equality_##K##_##V


struct hash_table_pair(K, V) {
    K key;
    V value;
};

hash_table_pair(K, V) hash_table_pair_create(K, V)(K key, V value) {
    return { key, value };
}
...
bool hash_table_simple_key_equality(K, V)(K* key_first, K* key_second) {
    return *key_first == *key_second;
}

stack_trace* hash_table_create(K, V)(hash_table(K, V)* table,
                                     uint32_t (*key_hash_function) (K key),
                                     size_t bucket_capacity = 32,
                                     size_t value_list_size = 10, 
                                     bool (*key_equals_function) (K* first, K* second) =
                                        hash_table_simple_key_equality(K, V)) {
    ...
    *table = {
    ...
    table->hash_table = calloc(bucket_capacity, sizeof(hash_table_bucket));
